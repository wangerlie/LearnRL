...
from .wrap import WrapMinariDataset
...
class SequenceDataset(Dataset):
    ...
    def __init__(self, minari_dataset=None, horizon=64,
        normalizer='LimitsNormalizer', preprocess_fns=[],
        termination_penalty=0, use_padding=True, 
        discount=0.99, returns_scale=1000, include_returns=False):
        ...
        # TODO: auto get max_path_length
        self.max_path_length = 1000
        fields = WrapMinariDataset(minari_dataset)
        self.discounts = self.discount ** np.arange(len(fields['path_lengths']))[:, None]
        self.normalizer = DatasetNormalizer(fields, normalizer, path_lengths=fields['path_lengths'])
        self.indices = self.make_indices(fields['path_lengths'], horizon)

        self.observation_dim = fields['observations'][0].shape[-1]
        self.action_dim = fields['actions'][0].shape[-1]
        self.fields = fields
        self.total_episodes = fields.total_episodes
        self.total_steps = fields.total_steps
        self.normalize()
    ...
    def make_indices(self, path_lengths, horizon):
        ...
        for i, path_length in enumerate(path_lengths):
            max_start = min(path_length - 1, path_length - horizon)
    ...
    def __getitem__(self, idx, eps=1e-4):
        path_index, start, end = self.indices[idx]

        observations = self.fields.normed_observations[path_index, start:end]
        actions = self.fields.normed_actions[path_index, start:end]

        conditions = self.get_conditions(observations)
        trajectories = np.concatenate([actions, observations], axis=-1)

        if self.include_returns:
            rewards = self.fields.rewards[path_index, start:]
...
class CondSequenceDataset(Dataset):
...
if __name__ == '__main__':
    ...
    print(sequence_data[0].trajectories.shape)
    print(sequence_data[0].conditions)
