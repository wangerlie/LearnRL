...
def extract(a, t, x_shape):
    ...
    Args:
        ...
        x_shape (tuple): The shape to which the output tensor should be reshaped.
...
def apply_conditioning(x, conditions, action_dim):
    """
    Apply conditioning to the input tensor `x` based on the provided conditions.

    Parameters:
    x (torch.Tensor): The input tensor to be conditioned.
    conditions (dict): A dictionary where keys are time steps and values are tensors 
                       to be applied at the corresponding time steps.
    action_dim (int): The dimension index from which the conditioning values should be applied.

    Returns:
    torch.Tensor: The conditioned tensor.
    """
